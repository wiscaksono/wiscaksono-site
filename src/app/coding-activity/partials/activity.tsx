    ...
    <FadeIn>
      <article>
        <div className='md:space-y-2 mb-2.5 pb-2.5 border-b'>
          <h1 className='md:text-3xl text-2xl font-semibold'>Weekly Activity</h1>
        ...
        {data.reverse().map(item => (
          <pre className='flex items-center' key={item.range.date}>
            <code className='text-muted-foreground w-40 truncate shrink-0'>{item.range.text}</code>
            ...
            <code className='w-44 text-end text-muted-foreground ml-4 md:ml-0'>{item.grand_total.text}</code>
          </pre>
        ))}
      </article>
    </FadeIn>
